    ...
    <div class="hidden lg:grid grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-r from-primary-50 to-white rounded-xl p-4 border border-primary-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-gray-800">{{ config('currencies.'.Auth::user()->currency, '$') }} {{ number_format(Auth::user()->account_bal,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-green-50 to-white rounded-xl p-4 border border-green-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-green-700">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format($monthly_deposits ?? 0,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-red-50 to-white rounded-xl p-4 border border-red-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-red-700">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format($monthly_expenses ?? 0,0, '.', ',') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-purple-50 to-white rounded-xl p-4 border border-purple-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-lg font-bold text-purple-700">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format(Auth::user()->limit, 2, '.', ',') }}</p>
            </div>
        ...
        <div class="font-medium text-white">{{ Auth::user()->name }}</div>
                    ...
                    <div class="mb-6">
                        ...
                        <div x-show="balanceVisible" x-transition class="text-3xl font-bold">
                            {{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format(Auth::user()->account_bal,0, '.', ',') }} {{ Auth::user()->currency }}
                        </div>
                        <div x-show="!balanceVisible" x-transition class="text-3xl font-bold">
                            *******
                        </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @forelse($allTransactions as $transaction)
                                    <tr class="hover:bg-gray-50 cursor-pointer" id="{{ $transaction->id }}{{ $transaction->txn_id }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format($transaction->amount, 2, '.', ',') }} {{ Auth::user()->currency }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format(Auth::user()->limit, 2, '.', ',') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format($total_deposited_pending + $total_withdrawal_pending, 2, '.', ',') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-lg font-bold text-gray-900 truncate">{{ config('currencies.'.Auth::user()->currency, '$') }}{{ number_format($total_withdrawal+$deposited, 2, '.', ',') }}</p>
                        </div>
                    </div>
            ...
            <div 
                ...
                <div class="flex items-start p-4 bg-primary-50 rounded-lg">
                    ...
                    <p class="text-sm text-gray-700">
                        Payment reference helps {{$settings->site_name}} track payments faster. Please include it in bank transfer description.
                    </p>
                </div>
...
_smartsupp.key = 'a53bf0720badcd3635f0951e10a3379e1d3cda15';
window.smartsupp||(function(d) {
  var s,c,o=smartsupp=function(){ o._.push(arguments)};o._=[];
  s=d.getElementsByTagName('ipt')[0];c=d.createElement('script');
